...
// Custom loader with progress bar for payment & data saving process
function showLoader() {
  if (!document.getElementById("loadingSpinner")) {
    const loader = document.createElement("div");
    loader.id = "loadingSpinner";
    ...
    loader.innerHTML = `
      <div style="width: 60px; height: 60px; border: 5px solid white; border-top-color: #FF0000; border-radius: 50%; animation: spin 1s linear infinite;"></div>
      <p id="loadingMessage" style="margin-top: 15px; font-size: 20px; font-weight: bold;">Uploading Images... Please Wait</p>
    ...
    console.log("✅ Engaging Loader with Progress Bar");
  }
}

// Function to update the loader's message dynamically
...
function loadCheckoutData() {
  setTimeout(() => {
    ...
    hideLoader(); // Stop loader when everything is loaded
  }, 1000);
}

// ✅ Check Authentication + Load Checkout Data with Loader
startLoader(); // Start loader before everything
    ...
    const options = {
      ...
      handler: async function(response) {
        try {
          ...
          // Hide loader and redirect to Order Success Page
          hideLoader();
...
// ✅ Retrieve Payment Details (For any later use)
